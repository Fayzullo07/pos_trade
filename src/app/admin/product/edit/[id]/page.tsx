"use client"
import { categoryGetAPI, productGetOneAPI, productPutAPI } from "@/api/AdminRequest";
...
const EditProduct = ({ params }: { params: any }) => {
    const { id } = params;
    ...
    const mutation = useMutation(
        {
            mutationFn: async () => {
                return productPutAPI(formData, id);
            },
            onSuccess: () => {
    ...
    const { data, isLoading, isError } = useQuery({
        queryKey: ["product"],
        queryFn: async () => {
            return await productGetOneAPI({id});
        }
    });
